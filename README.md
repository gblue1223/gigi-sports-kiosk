...
- 새 예약: 서버에서 날짜·요금·빈 타석을 조회하고 원하는 타석 또는 자동 배정을 선택합니다. 예약 확인 화면에 선택한 타석을 표시하며 저장 성공 후 예약 번호를 표시합니다. 타석 선택 API를 사용하려면 CMS의 `003_kiosk_bay_selection.sql`까지 적용해야 합니다.
- CMS 운영 정책: `004_booking_operations.sql` 적용 후 타석 사용 중지·운영시간·점검 일정·자동 배정 기준·전화번호 예약 제한이 신규 예약에 적용됩니다. `/reservations/policies`에서 설정한 최소·최대 인원도 인원 선택 화면에 반영됩니다. 기존 DB에는 1~4명 기본값을 사용합니다.
- 이용시간 변경: 60분/90분 전체 구간의 타석 여유를 다시 확인합니다.
- 통신 오류: 같은 화면에서 다시 예약 확정을 눌러 기존 요청의 결과를 확인합니다. 중복 터치를 차단합니다.
- 내 예약 확인: 휴대폰 번호만 입력하면 연결된 매장의 예약을 최근순으로 최대 50건 조회합니다.
